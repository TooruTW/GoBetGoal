...
export default function Calender(props: acceptProps) {
  ...
  const updateCurrentList = useCallback((arr: Challenge[],currentList:dayBoxType[]):dayBoxType[] => {
    const newList: dayBoxType[] = arr.map((item) => {
      return {
        date: new Date(item.deadline).getDate(),
        isThisMonth: (new Date(item.deadline).getMonth() === month && new Date(item.deadline).getFullYear() === year),
  ...
  // };
    ...
    useEffect(() => {
      ...
      const headList = new Array(firstDay)
        .fill(0)
        .map((_, index) => ({
      ...
      const tailList = new Array(7 - lastDay - 1)
        .fill(0)
        .map((_, index) => ({
          date: index + 1,
          isThisMonth: false,

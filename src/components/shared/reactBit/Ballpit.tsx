...
class X {
  ...
  private updateRenderer(): void {
    ...
    this.postprocessingRenderer?.setSize(this.size.width, this.size.height);
  ...
  get postprocessing(): PostprocessingRenderer | undefined {
    return this.postprocessingRenderer;
  }

  set postprocessing(value: PostprocessingRenderer | undefined) {
    this.postprocessingRenderer = value;
    if (value) {
  ...
  dispose(): void {
    ...
    this.stopAnimation();
    this.clear();
    this.postprocessingRenderer?.dispose();
    this.renderer.dispose();
    this.isDisposed = true;
  }

  private onResizeCleanup(): void {
...
interface CreatePointerDataOptions extends Partial<PointerData> {
...
function createBallpit(
  ...
  function initialize(cfg: Partial<BallpitConfig>): void {
    if (spheres) {
      threeInstance.clear();
  ...
  threeInstance.onBeforeRender = (deltaInfo: AnimationState) => {
    if (!isPaused) spheres.update(deltaInfo);
  };

  threeInstance.onAfterResize = (size: SizeData) => {

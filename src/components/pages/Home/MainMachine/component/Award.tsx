...
export default function Ranking() {
  ...
  return (
    <div className="  py-6 md:flex  items-center w-full justify-center">
      {/* 🔹 Top 3 排名 */}
      <div className="flex   sm:flex-nowrap gap-2 md:gap-4 md:w-2/3  mb-8 justify-center">
        {rankingList.slice(0, 3).map((item, index) => (
          <div
            key={item.user_id}
            className="flex flex-col items-center justify-center   w-1/3 text-nowrap"
          >
      ...
      {/* 🔹 4 ~ 10 排名 */}
      <div className="    text-nowrap md:w-1/3 flex flex-wrap">
        {rankingList.slice(3, 10).map((item, index) => (
          <div
            key={item.user_id}
            className="flex items-center gap-2 p-3 min-w-1/2 "
          >
